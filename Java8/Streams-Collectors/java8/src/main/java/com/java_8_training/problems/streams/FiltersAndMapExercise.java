...
together=input.flatMap(x->x.stream()).collect(toList());
    ...
    public static int product(){
        ...
        int product = 1;
        //TODO #8
product=input.reduce(0,(var, x)->var+x);
        return product;
    }
    ...
    public static Optional<Person> findFirstAllMalePersons(List<Person> personList){
        ...
        //TODO #9
Optional<Person>personOptional1=personList.stream().filter(Male->Male.isMale()).findFirst();
    ...
    public static boolean areAllPersonsBelowAge(List<Person> personList, Integer age){
        ...
        overAge=personList.stream().allMatch(x->x.getAge()<70);
    ...
    public static int findTheYoungestPerson(List<Person> personList){
        ...
        int min = 0;
        int min2 =0;
        min2=personList.stream().map(person->person.getAge()).reduce(Integer.MAX_VALUE,(acc,x)->Integer.min(acc,x));
    ...
    public static double findAgeAverage(List<Person> personList){
        double sum = 0;
        //TODO #13
        sum=personList.stream().collect(summingInt(x->x.getAge()));
...
persons=personList.stream().filter(s->s.getName().startsWith("j")).distinct().map(x->x.getNationality()).collect(toList());
    ...
    public static Optional<Integer> findSquareDivisibleBy3(){
        ...
        firstSquareDivisibleByThree=someNumbers.stream().filter(s->(Math.sqrt(s)%3==0)).findFirst();
    ...
    public static int sumStringLength(){
        ...
        sum=strings.stream().collect(summingInt(x->x.length()));
        //TODO #16
    ...
    public static  int sumSalary(List<Person> personList){
       ...
       sum= personList.stream().collect(summingInt(x->x.getSalary()));
        //TODO #17
